...
from src.torch.et_models.connecte import ConnectE
...
class connecte_trainer:
    ...
    def init(self, args, kgs):
        ...
        self.model = ConnectE(self.args, self.kgs)
        self.model.init()
    ...
    def launch_triple_training_1epo(self, epoch, triple_steps, steps_tasks, training_batch_queue, neighbors1, neighbors2):
        ...
        random.shuffle(self.kgs.relation_triples_list)
        random.shuffle(self.kgs.relation_triples_list)
    ...
    def launch_mapping_training_1epo(self, epoch, triple_steps):
        ...
        for i in range(triple_steps):
            ...
            neg_type = random.sample(self.kgs.type_list, len(self.kgs.train_et_list) // triple_steps)
            batch_loss = self.model.generate_mapping_loss({'ent': to_var(np.array([x[0] for x in links_batch]), self.device),
                                                           'type': to_var(np.array([x[1] for x in links_batch]), self.device),
                                                           'ent2': to_var(np.array([x[0] for x in links_batch]), self.device),
                                                           'type2': to_var(np.array([x for x in neg_type]), self.device)
                                                           })
            batch_loss.backward()
            self.optimizer.step()
            epoch_loss += batch_loss.item()
    ...
    def test(self):
        rest_12 = self.model.test(self.kgs.test_et_ent, self.kgs.test_en_type)
