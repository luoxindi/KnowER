...
class TransE_ET(BasicModel):
    ...
    self.type_embeddings = nn.Embedding(3584, self.dim)
	...
	def get_embeddings(self, hid, rid, tid):
		...
		candidates = self.type_embeddings.weight.data.view(1, 3584, self.dim)  # shape: (b_size, self.n_rel, self.emb_dim)
		candidates = candidates.expand(b_size, 3584, self.dim)
